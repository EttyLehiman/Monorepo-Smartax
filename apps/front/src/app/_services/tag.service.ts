...
export class TagService {
  ...
  createTag(tag: Tag): Observable<Tag> {
    return this.http.post<Tag>(this.apiUrl, tag, this.httpOptions)
      .pipe(
        catchError(this.handleError<Tag>('createClient'))
      );
  }
  ...
  getAllTags(): Observable<Tag[]> {
    return this.http.get<Tag[]>(`${this.apiUrl}`)
      .pipe(
        catchError(this.handleError<Tag[]>('getAllClients', []))
      );
  }

  // Search for a Client by ID
  searchTag(id: string): Observable<Tag[]> {
    return this.http.post<Tag[]>(`${this.apiUrl}/searchClient`, { id }, this.httpOptions)
      .pipe(
        catchError(this.handleError<Tag[]>('searchClient', []))
      );
  }

  // Update an existing Client
  updateTag(client: Tag): Observable<Tag> {
    return this.http.put<Tag>(`${this.apiUrl}`, client, this.httpOptions)
      .pipe(
        catchError(this.handleError<Tag>('updateClient'))
      );
  }
  ...
  deleteTag(id: string): Observable<boolean> {
    return this.http.delete<boolean>(`${this.apiUrl}`, { ...this.httpOptions, body: { id } })
      .pipe(
        catchError(this.handleError<boolean>('deleteTag', false))
      );
  }
